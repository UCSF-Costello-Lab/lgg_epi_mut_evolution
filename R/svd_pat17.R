...
xlab_cols <- c(cols[4], xlab_cols_map[c(1,1,1,2,3,2,3)])


p1 <- 1; t1 <- 220; p2 <- 2; t2 <- 0
cat(row.names(dat[which(rank(-abs(x[,p1])) <= t1),]), sep = '\n', file = paste0('results/p17_PC', p1, '_',t1, '.txt'))
#cat(row.names(dat[which(rank(-abs(x[,p2])) <= t2),]), sep = '\n', file = paste0('p17_PC', p2, '_',t2, '.txt'))
#p1 <- 3; t1 <- 0; p2 <- 4; t2 <- 50
#cat(row.names(dat[which(rank(-abs(x[,p1])) <= t1),]), sep = '\n', file = paste0('p17_PC', p1, '_',t1, '.txt'))
#cat(row.names(dat[which(rank(-abs(x[,p2])) <= t2),]), sep = '\n', file = paste0('p17_PC', p2, '_',t2, '.txt'))
#p1 <- 5; t1 <- 20;   p2 <- 6; t2 <- 10
#cat(row.names(dat[which(rank(-abs(x[,p1])) <= t1),]), sep = '\n', file = paste0('p17_PC', p1, '_',t1, '.txt'))
#cat(row.names(dat[which(rank(-abs(x[,p2])) <= t2),]), sep = '\n', file = paste0('p17_PC', p2, '_',t2, '.txt'))
#p1 <- 7; t1 <- 0;   p2 <- 8; t2 <- 0
#cat(row.names(dat[which(rank(-abs(x[,p1])) <= t1),]), sep = '\n', file = paste0('p17_PC', p1, '_',t1, '.txt'))
