...
sig_probes <- scan('results/p17_PC1_220.txt', what = '')
...
sig_genes <- unique(subset(anno, IlmnID %in% sig_probes)$gene_name)
e_enrich <- gene_entrez_link[,2][which(gene_entrez_link[,1] %in% sig_genes)]
go <- enrichGO(gene = as.character(e_enrich),  readable = T, pvalueCutoff = .2, ont = "BP", universe = e_bg)
if(!is.null(go)){
  write.table(summary(go), file = 'results/p17_PC1_genomic_promoter_genes_go_table.txt' , sep = '\t', row.names = F)
}
