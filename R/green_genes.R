...
cond_rna <- with(log_df, which(temp < p_cutoff & (g4_fc_log_2 <=log(1/2,2)) &  (g4_fc_log_2 - g2_fc_log_2) < -1))
cond_methyl <- with(logit_df,which(p_methyl_adjusted < p_cutoff & (g4_beta_d >= .2)  & (g4_beta_d - g2_beta_d) > .15))


dat <- log_df[cond_rna,  ]
methyl_dat <- logit_df[cond_methyl ,  ]  

cat(as.character(rownames(methyl_dat)), sep = '\n', file = 'results/sig_probes_green.txt')
